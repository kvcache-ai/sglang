...
def main():
    ...
    print("=" * 80)
    print("Transformers Layer Output Dump")
    print("=" * 80)

    # 1. 加载 tokenizer
    print(f"\n[1/4] Loading tokenizer from {args.model_path}...")
    ...
    # 2. 加载 base model
    print(f"[2/4] Loading base model from {args.model_path}...")
    ...
    # 3. 加载 LoRA（如果指定）
    if args.lora_path:
        print(f"[3/4] Loading LoRA adapter from {args.lora_path}...")
        ...
        print(f"[3/4] No LoRA adapter specified, using base model")
        output_filename = "transformers_base_outputs.pkl"

    model.eval()

    # 4. 注册 hooks
    print(f"[4/4] Registering hooks...")
