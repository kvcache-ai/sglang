...
class KVManager:
    ...
    def send_kvcache(self, endpoint: str, bootstrap_room: int, dst_ptrs: list[int], dst_kv_indices: npt.NDArray[np.int32]):
    ...
    def start_prefill_thread(self):
        ...
        def prefill_thread():
            while True:
                (endpoint, bootstrap_room, dst_ptrs,
                dst_kv_indices, dst_aux_ptrs, dst_aux_index) = self.server_socket.recv_multipart()
                ...
                dst_aux_ptrs = list(struct.unpack(f'{len(dst_aux_ptrs)//8}q', dst_aux_ptrs))
                ...
                self._connect("tcp://" + endpoint + ":" + str(KVRECIVER_POLLING_PORT + self.kv_args.engine_rank)).send_multipart(
                    [
...
class KVReceiver:
    def __init__(
        ...
        self.prefill_server_url = (
            bootstrap_addr.split(":")[0] + ":" + str(KVSENDER_POLLING_PORT + self.kv_mgr.kv_args.engine_rank)
        )
