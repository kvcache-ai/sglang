...
class SharedFullContext:
    ...
    def _build_layers(
        ...
        if "weight_loader_fused" in local_extra_weight_attrs and hasattr(
            ...
            local_extra_weight_attrs["weight_loader_fused"] = (
                self.gpu_layer.weight_loader_fused
            )
    ...
    def _collect_all_rank_buffer_pointers(self) -> Dict[str, List[int]]:
        ...
        logger.info(
            "Rank %d: Collected buffer pointers from %d ranks", tp_rank, tp_world_size
        )
    ...
    def cleanup_shared_memory(self):
        ...
        if torch.cuda.is_available():
            for name, cpu_buffer in self.cpu_buffers.items():
                try:
                    ...
                    logger.warning(
                        "Rank %d: Failed to unregister pinned memory '%s': %s",
                        tp_rank,
                        name,
                        e,
                    )
        ...
        if hasattr(self, "_opened_shm_refs"):
            for key, shm in self._opened_shm_refs.items():
                try:
                    ...
                    logger.warning(
                        "Rank %d: Failed to close shared memory ref '%s': %s",
                        tp_rank,
                        key,
                        e,
                    )
        ...
        for name, shm in self.shm_handles.items():
            try:
                ...
                logger.warning(
                    "Rank %d: Failed to unlink shared memory '%s': %s", tp_rank, name, e
                )
...
class KTEPWrapperMethod(FusedMoEMethodBase):
    ...
    def __init__(
        ...
        method_factory: Optional[
            Callable[[torch.nn.Module], FusedMoEMethodBase]
        ] = None,
    ):
    ...
    def _build_full_context(self, layer: torch.nn.Module) -> _FullMoEContext:
