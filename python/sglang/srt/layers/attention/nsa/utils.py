...
def nsa_use_prefill_cp(forward_batch, nsa_enable_prefill_cp=None):
    ...
    if (
        ...
        if envs.SGLANG_DEBUG_HACK_CP_ASSERT_PURE_EXTEND.get():
            from sglang.srt.layers.attention.nsa.utils_v4 import assert_cp_pure_extend

            assert_cp_pure_extend(forward_batch)
        return True
    else:
        return False
...
def cp_all_gather_rerange_output(input_tensor, cp_size, forward_batch, stream):
    ...
    if is_nsa_prefill_cp_round_robin_split():
        ...
        if envs.SGLANG_OPT_CP_REARRANGE_TRITON.get():
            from sglang.srt.layers.attention.nsa.utils_v4 import CpRoundRobinRerange
